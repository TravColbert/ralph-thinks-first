...
1. The source code file name will be specified to you as: `SOURCE_CODE_FILE_NAME`
2. You will create a Markdown file in the `/documents` folder off of the current working directory with a name of: `${SOURCE_CODE_FILE_NAME}.md`. For example, a SOURCE_CODE_FILE_NAME of `sourceCode.js` would have an accompanying documentation file of: `/documents/sourceCode.js.md`
3. If the `/documents` folder does not exist, you are free to create it.
4. The main heading of the Markdown file should be the file path name of the SOURCE CODE FILE.
5. Provide an `## OVERVIEW` section that provides a simple, overall explanation of the SOURCE CODE FILE.
6. You may have to read other files in the current working directory or its subfolders to fully understand the meaning of the SOURCE_CODE_FILE.
7. Each major logical block in the SOURCE CODE FILE can have its own Markdown heading (`##`)
8. If there are any spurious elements in the code (unnecessary modules, unused variables, unused functions or methods, references to missing functions or modules, unusual or unecessary elements in function signature) you will enumerate these in a special section near the top of the Markdown file called: `## INVESTIGATE and CLARIFY`
